...
//------------------------------------------------------------------------- INITIALIZATION -----------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#include <Arduino.h>            // Importing Arduino library
...
#define SDI12_SENSOR_ADDRESS 0    // Defines SDI-12 water sensor address
float HG_OFFSET;                // Defines the offset to the HG parameter
...
DS3231_Simple Clock;            // Initializing the RTC
...
void setup() {
  ...
  delay(1000);        // Sending 1 second delay to allow bus time to boot-up
...
ONLY use "live", "log", and "menu" the last command is for collecting data off of the datalogger
*/
void loop() {
  ...
  HG_OFFSET = readFloatFromEEPROM(2); // reading the HG_OFFSET off of EEPROM address 2

  // ADD THE VARIABLES BEING MEASURED AS FLOATS HERE:
  float HG;    // Water level taken off of water level sensor
  ...
  if (CurrentTime.Minute == LogTime) {
    ...
    HG = Measurement_Output(SDI12_SENSOR_ADDRESS, 1, 0.704, HG_OFFSET);   // 1st measurement on PT12_SENSOR_ADDRESS
    //TW = Measurement_Output(SDI12_SENSOR_ADDRESS, 2, 0.704, HG_OFFSET); // 2nd measurement on PT12_SENSOR_ADDRESS
    ...
    DataString += String(HG) + "\t" + String(VB);
