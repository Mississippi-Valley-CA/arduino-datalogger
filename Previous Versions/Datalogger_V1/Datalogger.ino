...
DateTime timestamp;
int mins = 16;                // Set starting interval (round up to the nearest 10 min)
...
// Clock Setup
  ...
  //alarm every 10 minutes -> needs a given minute time stamp
  timestamp = Clock.read();
  timestamp.Minute = mins;
  Clock.setAlarm(timestamp, DS3231_Simple::ALARM_MATCH_MINUTE);
  // Clock.setAlarm(MyTimestamp, DS3231_Simple::ALARM_MATCH_MINUTE); // Setting the alarm for every ten minutes (defined as 0th minute)
