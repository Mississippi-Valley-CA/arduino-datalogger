...
#define Serial1_BAUD 9600        // Serial1 COM rate
...
String incomingByte;                        // Serial1 input variable (User input)
...
void setup() {
  // Serial1 setup
  Serial1.begin(Serial1_BAUD);                   // Begin Serial1 communication
  ...
  // }
  // file_write(DAILYDATA, "Date\tTime\tHG\tTW\tTA");  // Writing the headings to the file define DAILYDATA: (ADD PARAMETERS THAT WILL BE LOGGED)

  //   if(SD.exists(ORIGINALDATA)){
  //     SD.remove(ORIGINALDATA);     // Resetting the original data
  //   }
  //   file_write(ORIGINALDATA, "Date\tTime\tHG\tTW\tTA");   // Writing the headings to the file define ORIGINALDATA: (ADD PARAMETERS THAT WILL BE LOGGED)
...
void loop() {
// This string will be sent to the SD card or the Serial1
...
// If there is a Serial1 input take the input into a integer ( will only take numbers -> floats become integers )   
  if (Serial1.available() > 0) {
    incomingByte = Serial1.readStringUntil('\n');
    incomingByte.trim();  // Remove leading/trailing whitespaces

// WILL TAKE A LIVE READING AND PRINT TO Serial1 MONITOR IF INPUT IS 1100
    if(incomingByte == "live"){
      
      DateTime MyTimestamp = Clock.read();  // Taking a clock reading
      DataString = Date_time(MyTimestamp);  // Adding the date and time to the string that will be printed to the Serial1 monitor
...
// Adding the parameters to thestring that will be printed to Serial1( should be same as logging to sd card )
      DataString += String(VWC) + "\t" + String(SLTMP) + "\t" + String(VB);
...
// WILL PRINT ALL THE DATA FROM THE DEFINED ORIGINALDATA FILE AND PRINT TO Serial1 MONITOR IF INPUT IS 1111 (Mainly used for complete data dump)
      else if (incomingByte == "log"){
        data_output(ORIGINALDATA); // Outputing the daat to Serial1 monitor       
      }
...
// WILL PRINT THE DAILY DATA FROM THE DEFINED DAILYDATA FILE AND PRINT TO Serial1 MONITOR IF INPUT IS 1110 (Mainly used for getting data into WISKI)
      else if (incomingByte == "9999"){
        data_output(DAILYDATA);     // Outputing the daat to Serial1 monitor
