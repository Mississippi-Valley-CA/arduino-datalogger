...
void displayMenu() {
   ...
   Serial.println("6. Change HG_OFFSET to HG");
...
bool processChoice(int choice) {
  ...
  case 5:
    ...
    break;
  case 6:
     Serial.println("You selected Option 6: Change HG_OFFSET to HG");
    processOption6();
     Serial.println("Completed... Please select another option from the menu\n");
    break;
  case 7:
...
// Change the date
void processOption2() {
   Serial.println("Please Enter Date (yy-mm-dd):");
...
// ---------------------------------------------------------------- OPTION 6 ------------------------------------------------------------------
// Changing the parameter HG_OFFSET
void processOption6() {
  HG_OFFSET = readFloatFromEEPROM(2);
   Serial.println("Current HG_OFFSET:\t" + String(HG_OFFSET));
   Serial.println("Please Enter New HG_OFFSET:");
  while (! Serial.available()) {
    // Wait for user input
  }
  String newOFFSET =  Serial.readStringUntil('\n');
  HG_OFFSET = newOFFSET.toFloat();
  writeFloatToEEPROM(2, HG_OFFSET);
  HG_OFFSET = readFloatFromEEPROM(2);
   Serial.println("\nNew HG_OFFSET:\t" + String(HG_OFFSET));
}
